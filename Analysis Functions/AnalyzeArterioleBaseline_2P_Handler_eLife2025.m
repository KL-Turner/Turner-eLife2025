...
cc = 1;
multiWaitbar('Analyzing arteriole baseline for 2P',0,'Color','P');
for aa = 1:length(groups)
    folderList = dir([groups{1,aa} delim set]);
    folderList = folderList(~startsWith({folderList.name},'.'));
    ...
    for bb = 1:length(animalIDs)
        if isfield(Results_Baseline_2P.(groups{1,aa}),animalIDs{1,bb}) == false
            [Results_Baseline_2P] = AnalyzeArterioleBaseline_2P_eLife2025(animalIDs{1,bb},groups{1,aa},set,rootFolder,delim,Results_Baseline_2P);
        end

function [] = ExtractHeartRate_IOS_nNOS(procDataFileIDs)
%----------------------------------------------------------------------------------------------------------
    ...
    if isfield(ProcData.data,'heartRate') == false
        ...
        if ProcData.notes.wavelengthSamplingRate >= 30
            if strcmpi(imagingType,'Single ROI (SI)') == true
                [~,~,~,HR] = FindHeartRate_IOS_nNOS(ProcData.data.(wavelength).barrels,ProcData.notes.SamplingRate);
            elseif strcmpi(imagingType,'Single ROI (SSS)') == true
                [~,~,~,HR] = FindHeartRate_IOS_nNOS(ProcData.data.(wavelength).SSS,ProcData.notes.wavelengthSamplingRate);
            elseif strcmpi(imagingType,'Bilateral ROI (SI)') == true || strcmpi(imagingType,'Bilateral ROI (SI,FC)') == true
                ...
                [~,~,~,LH_HR] = FindHeartRate_IOS_nNOS(ProcData.data.(wavelength).LH,ProcData.notes.wavelengthSamplingRate);
                [~,~,~,RH_HR] = FindHeartRate_IOS_nNOS(ProcData.data.(wavelength).RH,ProcData.notes.wavelengthSamplingRate);

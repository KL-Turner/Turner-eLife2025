function [procDataFileIDs] = ProcessRawDataFiles_IOS_nNOS()
%----------------------------------------------------------------------------------------------------------
...
for a = 1:size(rawDataFileIDs,1)
    ...
    [animalID,fileDate,fileID] = GetFileInfo_IOS_nNOS(rawDataFile);
    ...
    if ~exist(procDataFile,'file') == true
        ...
        for c = 1:length(neuralDataTypes)
            ...
            % MUA [300 - 3000]
            [muaPower,~] = ProcessNeuro_IOS_nNOS(RawData,analogExpectedLength,'MUA',neuralDataType);
            ProcData.data.(neuralDataType).muaPower = muaPower;
            % gamma [30 - 100]
            [gammaBandPower,~] = ProcessNeuro_IOS_nNOS(RawData,analogExpectedLength,'Gam',neuralDataType);
            ProcData.data.(neuralDataType).gammaBandPower = gammaBandPower;
            % beta [13 - 30 Hz]
            [betaBandPower,~] = ProcessNeuro_IOS_nNOS(RawData,analogExpectedLength,'Beta',neuralDataType);
            ProcData.data.(neuralDataType).betaBandPower = betaBandPower;
            % alpha [10 - 13 Hz]
            [alphaBandPower,~] = ProcessNeuro_IOS_nNOS(RawData,analogExpectedLength,'Alpha',neuralDataType);
            ProcData.data.(neuralDataType).alphaBandPower = alphaBandPower;
            % theta [4 - 10 Hz]
            [thetaBandPower,~] = ProcessNeuro_IOS_nNOS(RawData,analogExpectedLength,'Theta',neuralDataType);
            ProcData.data.(neuralDataType).thetaBandPower = thetaBandPower;
            % delta [1 - 4 Hz]
            [deltaBandPower,~] = ProcessNeuro_IOS_nNOS(RawData,analogExpectedLength,'Delta',neuralDataType);
        ...
        [patchedWhisk] = PatchWhiskerAngle_IOS_nNOS(RawData.data.whiskerAngle,RawData.notes.whiskCamSamplingRate,RawData.notes.trialDuration_sec,RawData.notes.droppedWhiskCamFrameIndex);
        ...
        [ok] = CheckForThreshold_IOS_nNOS(['binarizedWhiskersLower_' strDay],animalID);
        if ok == 0
            [whiskersThresh1,whiskersThresh2] = CreateWhiskThreshold_IOS_nNOS(resampledWhisk,ProcData.notes.dsFs,strDay);
        ...
        binWhisk = BinarizeWhiskers_IOS_nNOS(resampledWhisk,ProcData.notes.dsFs,Thresholds.(['binarizedWhiskersLower_' strDay]),Thresholds.(['binarizedWhiskersUpper_' strDay]));
        [linkedBinarizedWhiskers] = LinkBinaryEvents_IOS_nNOS(gt(binWhisk,0),[round(ProcData.notes.dsFs/3),0]);
        ...
        [ok] = CheckForThreshold_IOS_nNOS(['binarizedForceSensor_' strDay],animalID);
        if ok == 0
            [forceSensorThreshold] = CreateForceSensorThreshold_IOS_nNOS(ProcData.data.forceSensor.force,ProcData.notes.dsFs,strDay);
            Thresholds.(['binarizedForceSensor_' strDay]) = forceSensorThreshold;
            save([animalID '_Thresholds.mat'],'Thresholds');
        end
        ProcData.data.forceSensor.binarization = [BinarizeForceSensor_IOS_nNOS(ProcData.data.forceSensor.force,Thresholds.(['binarizedForceSensor_' strDay])),0];
        ...
        % pupil data
        [ProcData] = PatchPupilData_IOS_nNOS(RawData,ProcData);
...
% character list of all ProcData files in the directory from ProcessRawDataFiles_IOS_nNOS.m

function [ROIs] = CheckROIDates_IOS_nNOS(animalID,ROIs,ROInames,lensMag,imagingType,imagingWavelengths,imagingCamera)
%----------------------------------------------------------------------------------------------------------
...
[~,fileDates,~] = GetFileInfo_IOS_nNOS(windowCamDataFileIDs);
...
for b = 1:length(firstsFileOfDay)
    ...
    strDay = ConvertDate_IOS_nNOS(fileID);
    for c = 1:length(ROInames)
        ...
        if ~isfield(ROIs,strDay) == true || ~isfield(ROIs.(strDay),ROIname) == true
            if any(strcmp(ROInames{1,c},{'LH','RH','fLH','fRH','barrels'})) == true
                ...
                [ROIs] = PlaceROIs_IOS_nNOS(animalID,fileID,ROIs,lensMag,imagingType,imagingWavelengths,imagingCamera);
            elseif strcmp(ROInames{1,c},{'SSS'}) == true
                % ROIS specifically for SSS
                [ROIs] = DrawSagSinusROIs_IOS_nNOS(animalID,fileID,ROIs,imagingWavelengths,imagingCamera);
            else
                % any additional free hand ROIs
                [ROIs] = CreateFreeHandROIs_IOS_nNOS(animalID,fileID,ROIs,imagingWavelengths,imagingCamera);
            end

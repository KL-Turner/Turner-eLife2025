function [EventData] = ExtractEventTriggeredData_IOS_nNOS(procDataFileIDs)
%----------------------------------------------------------------------------------------------------------
...
[dataTypes] = DetermineWavelengthDatatypes_IOS_nNOS(imagingWavelengths,3);
for aa = 1:length(dataTypes)
    ...
    for bb = 1:size(procDataFileIDs,1)
        ...
        [~,fileDate,fileID] = GetFileInfo_IOS_nNOS(procDataFileIDs(bb,:));
        ...
        for cc = 1:length(subDataTypes)
            ...
            for dd = 1:length(behaviorFields)
                ...
                try
                    [chunkdata,evFilter] = ExtractBehavioralData_IOS_nNOS(data,epoch,sDT,behaviorFields{dd},samplingRate);
                catch
                    chunkdata = [];
                    evFilter = [];
                end
                % add epoch details to temp struct
                [temp] = AddEpochInfo_IOS_nNOS(data,sDT,behaviorFields{dd},temp,fileID,fileDate,evFilter,bb);
    ...
    [EventData] = ProcessTempStruct_IOS_nNOS(EventData,dataType,temp,epoch);
end
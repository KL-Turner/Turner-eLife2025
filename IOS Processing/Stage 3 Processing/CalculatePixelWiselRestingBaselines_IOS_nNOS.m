function [RestingBaselines] = CalculatePixelWiselRestingBaselines_IOS_nNOS(procDataFileIDs,RestingBaselines)
%----------------------------------------------------------------------------------------------------------
...
if isfield(RestingBaselines,'Pixel') == false
    ...
    for bb = 1:length(restFileList)
        ...
        strDay = ConvertDate_IOS_nNOS(fileID(1:6));
    ...
    uniqueDays = GetUniqueDays_IOS_nNOS(RestingBaselines.manualSelection.baselineFileInfo.fileIDs);
    for aa = 1:length(wavelengths)
        wavelength = wavelengths{1,aa};
        clear frameAvgs
        for f = 1:length(uniqueDays)
            g = 1;
            stringDay = ConvertDate_IOS_nNOS(uniqueDays{f});

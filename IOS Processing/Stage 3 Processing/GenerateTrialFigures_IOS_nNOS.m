function [] = GenerateTrialFigures_IOS_nNOS(procDataFileIDs,RestingBaselines)
%----------------------------------------------------------------------------------------------------------
...
for aa = 1:size(procDataFileIDs,1)
    ...
    if strcmpi(imagingType,{'Single ROI (SI)'}) == true
        [figHandle] = GenerateTrialFigures_SI_IOS_nNOS(procDataFileID);
    elseif strcmpi(imagingType,'Single ROI (SSS)') == true
        [figHandle] = GenerateTrialFigures_SSS_IOS_nNOS(procDataFileID,RestingBaselines);
    elseif any(strcmpi(imagingType,{'Bilateral ROI (SI)','Bilateral ROI (SI,FC)'})) == true
        [figHandle] = GenerateBilateralTrialFigures_IOS_nNOS(procDataFileID);
    end

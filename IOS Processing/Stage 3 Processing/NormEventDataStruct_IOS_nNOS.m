function [EventData] = NormEventDataStruct_IOS_nNOS(animalID,EventData,RestingBaselines,baselineType)
%----------------------------------------------------------------------------------------------------------
    ...
    for hDT = 1:length(hemisphereDataTypes)
        ...
        for bF = 1:length(behaviorFields)
            ...
            if isempty(EventData.(dataType).(hemDataType).(behavField).data) == false
                [uniqueDays,~,~] = GetUniqueDays_IOS_nNOS(EventData.(dataType).(hemDataType).(behavField).fileDates);
                for uD = 1:length(uniqueDays)
                    date = uniqueDays{uD};
                    strDay = ConvertDate_IOS_nNOS(date);
                    [~,dayInds] = GetDayInds_IOS_nNOS(EventData.(dataType).(hemDataType).(behavField).fileDates,date);

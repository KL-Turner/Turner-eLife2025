function [RestingBaselines,ManualDecisions,procDataFileIDs,animalID] = CalculateManualRestingBaselinesTimeIndeces_IOS_nNOS(hemoType)
%----------------------------------------------------------------------------------------------------------
...
for a = 1:size(procDataFileIDs,1)
    ...
    [animalID,~,~] = GetFileInfo_IOS_nNOS(procDataFileID);
    ...
    if isfield(ProcData,'manualBaselineInfo') == false
        ...
        while b == false
            ...
            [singleTrialFig] = GenerateSingleFigures_IOS_nNOS(procDataFileID,RestingBaselines,'setDuration',hemoType);
...
for e = 1:length(dataTypes)
    ...
    if strcmp(dataType,'Pupil') == false
        ...
        for f = 1:length(subDataTypes)
            ...
            [restLogical] = FilterEvents_IOS_nNOS(RestData.(dataType).(subDataType),RestCriteria);
            [stimLogical] = FilterEvents_IOS_nNOS(RestData.(dataType).(subDataType),StimCriteria);
            ...
            uniqueDays = GetUniqueDays_IOS_nNOS(RestData.(dataType).(subDataType).fileIDs);
            ...
            for p = 1:length(uniqueDays)
                ...
                for r = 1:length(finalEventFileIDs)
                    ...
                    uniqueDate{p,1} = ConvertDate_IOS_nNOS(uniqueDays{p,1});

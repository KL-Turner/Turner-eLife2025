function [RestData] = NormRestDataStruct_IOS_nNOS(animalID,RestData,RestingBaselines,baselineType)
%----------------------------------------------------------------------------------------------------------
...
for dT = 1:length(dataTypes)
    ...
    for hDT = 1:length(hemisphereDataTypes)
        ...
        normData = {};
        [uniqueDays,~,~] = GetUniqueDays_IOS_nNOS(RestData.(dataType).(hemDataType).fileDates);
        for uD = 1:length(uniqueDays)
            date = uniqueDays{uD};
            strDay = ConvertDate_IOS_nNOS(date);
            [~,dayInds] = GetDayInds_IOS_nNOS(RestData.(dataType).(hemDataType).fileDates,date);

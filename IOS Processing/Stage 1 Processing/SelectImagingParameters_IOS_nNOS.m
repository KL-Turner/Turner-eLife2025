function [imagingParameters,animalID,fileList] = SelectImagingParameters_IOS_nNOS()
%----------------------------------------------------------------------------------------------------------
...
imagingParameters.camera  = SelectImagingType_IOS_nNOS(cameraOptions);
% select imaging type
imagingOptions = {'Single ROI (SI)','Single ROI (SSS)','Bilateral ROI (SI)','Bilateral ROI (SI,FC)'};
disp('Please select the window type'); disp(' ')
imagingParameters.type = SelectImagingType_IOS_nNOS(imagingOptions);
% select imaging type
...
imagingParameters.wavelengths = SelectImagingType_IOS_nNOS(wavelengthOptions);
% select imaging type
pupilOptions = {'Yes','No'};
disp('Please select whether to run the pupil tracker'); disp(' ')
imagingParameters.pupilOptions = SelectImagingType_IOS_nNOS(pupilOptions);
% animal ID

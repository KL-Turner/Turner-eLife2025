function [AnalysisResults] = Fig3_HipTest_HRF2020(rootFolder,saveFigs,delim,AnalysisResults)
...
dataTypes = {'IR_gammaBandPower_hippLH','FM_gammaBandPower_hippLH','IR_muaPower_hippLH','FM_muaPower_hippLH'};
...
for dd = 1:length(neuralBands)
    ...
    for ee = 1:length(behaviors)
        ...
        data.Predictions.IR_R.(neuralBand).(behavior).cat = cat(1,data.Predictions.IR_R.(neuralBand).(behavior).hippLH);
        data.Predictions.IR_R2.(neuralBand).(behavior).cat = cat(1,data.Predictions.IR_R2.(neuralBand).(behavior).hippLH);
        data.Predictions.FM_R.(neuralBand).(behavior).cat = cat(1,data.Predictions.FM_R.(neuralBand).(behavior).hippLH);
        data.Predictions.FM_R2.(neuralBand).(behavior).cat = cat(1,data.Predictions.FM_R2.(neuralBand).(behavior).hippLH);
    end
end

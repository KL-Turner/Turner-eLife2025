function [AnalysisResults] = EvaluateCBVPredictionCoherence_HRF2020(animalID,neuralBand,hemisphere,behavior,AnalysisResults)
...
if strcmp(behavior,'Rest') == true || strcmp(behavior,'NREM') == true || strcmp(behavior,'REM') == true || strcmp(behavior,'All') == true || strcmp(behavior,'Alert') == true || strcmp(behavior,'Asleep') == true
    ...
    if isempty(ProcHemoTestData) == false
        ...
        [IR_C,~,~,~,~,IR_f,IR_confC,~,IR_cErr] = coherencyc_HRF2020(IR_truncPred,IR_truncAct,params);
        [FM_C,~,~,~,~,FM_f,FM_confC,~,FM_cErr] = coherencyc_HRF2020(FM_truncPred,FM_truncAct,params);
        % save results
        AnalysisResults.Coherence.(animalID).(neuralBand).(hemisphere).(behavior).IR_C = IR_C;
        AnalysisResults.Coherence.(animalID).(neuralBand).(hemisphere).(behavior).FM_C = FM_C;
        AnalysisResults.Coherence.(animalID).(neuralBand).(hemisphere).(behavior).IR_f = IR_f;
        AnalysisResults.Coherence.(animalID).(neuralBand).(hemisphere).(behavior).FM_f = FM_f;
        AnalysisResults.Coherence.(animalID).(neuralBand).(hemisphere).(behavior).IR_confC = IR_confC;
        AnalysisResults.Coherence.(animalID).(neuralBand).(hemisphere).(behavior).FM_confC = FM_confC;
        AnalysisResults.Coherence.(animalID).(neuralBand).(hemisphere).(behavior).IR_cErr = IR_cErr;
        AnalysisResults.Coherence.(animalID).(neuralBand).(hemisphere).(behavior).FM_cErr = FM_cErr;
    end
elseif strcmp(behavior,'Contra') == true || strcmp(behavior,'Whisk') == true
    ...
    % calculate the coherence between desired signals
    [IR_C,~,~,~,~,IR_f,IR_confC,~,IR_cErr] = coherencyc_HRF2020(TestData.IR_mPred,TestData.IR_mAct,params);
    [FM_C,~,~,~,~,FM_f,FM_confC,~,FM_cErr] = coherencyc_HRF2020(TestData.FM_mPred,TestData.FM_mAct,params);
    % save results
    AnalysisResults.Coherence.(animalID).(neuralBand).(hemisphere).(behavior).IR_C = IR_C;
    AnalysisResults.Coherence.(animalID).(neuralBand).(hemisphere).(behavior).FM_C = FM_C;
    AnalysisResults.Coherence.(animalID).(neuralBand).(hemisphere).(behavior).IR_f = IR_f;

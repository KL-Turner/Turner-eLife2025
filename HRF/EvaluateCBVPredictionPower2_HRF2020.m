function [AnalysisResults] = EvaluateCBVPredictionPower2_HRF2020(animalID,neuralBand,hemisphere,behavior,AnalysisResults)
...
if strcmp(behavior,'Rest') == true || strcmp(behavior,'NREM') == true || strcmp(behavior,'REM') == true || strcmp(behavior,'All') == true || strcmp(behavior,'Alert') == true || strcmp(behavior,'Asleep') == true
    ...
    for ff = 1:length(ProcHemoTestData)
        ...
        if strcmp(behavior,'Rest') == true || strcmp(behavior,'NREM') == true || strcmp(behavior,'REM') == true
            [Act,Pred] = ConvolveHRF_HRF2020(AnalysisResults.HRFs.(animalID).HbT.(neuralBand).(hemisphere).(behavior).IR_gammaFunction,detrend(ProcNeuralTestData{ff}),detrend(ProcHemoTestData{ff}),0);
        elseif strcmp(behavior,'All') == true || strcmp(behavior,'Alert') == true || strcmp(behavior,'Asleep') == true
    ...
    [pred_S,pred_f,pred_sErr] = mtspectrumc_HRF2020(truncPred,params);
    [act_S,act_f,act_sErr] = mtspectrumc_HRF2020(truncAct,params);
    % save results
    AnalysisResults.Power.(animalID).HbT.(neuralBand).(hemisphere).(behavior).pred_S = pred_S;
    AnalysisResults.Power.(animalID).HbT.(neuralBand).(hemisphere).(behavior).pred_f = pred_f;
    AnalysisResults.Power.(animalID).HbT.(neuralBand).(hemisphere).(behavior).pred_sErr = pred_sErr;
    AnalysisResults.Power.(animalID).HbT.(neuralBand).(hemisphere).(behavior).act_S = act_S;
    AnalysisResults.Power.(animalID).HbT.(neuralBand).(hemisphere).(behavior).act_f = act_f;
    AnalysisResults.Power.(animalID).HbT.(neuralBand).(hemisphere).(behavior).act_sErr = act_sErr;
elseif strcmp(behavior,'Contra') == true || strcmp(behavior,'Whisk') == true
    % test data
    for ii = 1:size(ProcHemoTestData,1)
        [Act,Pred] = ConvolveHRF_HRF2020(AnalysisResults.HRFs.(animalID).HbT.(neuralBand).(hemisphere).(behavior).IR_gammaFunction,ProcNeuralTestData(ii,:),ProcHemoTestData(ii,:),0);
        mPred = Pred(strt:stp) - mean(Pred(strt:stp));
        mAct = Act(strt:stp) - mean(Act(strt:stp));
    ...
    % calculate the coherence between desired signals
    [pred_S,pred_f,pred_sErr] = mtspectrumc_HRF2020(TestData.mPred,params);
    [act_S,act_f,act_sErr] = mtspectrumc_HRF2020(TestData.mAct,params);
    % save results
    AnalysisResults.Power.(animalID).HbT.(neuralBand).(hemisphere).(behavior).pred_S = pred_S;
    AnalysisResults.Power.(animalID).HbT.(neuralBand).(hemisphere).(behavior).pred_f = pred_f;
    AnalysisResults.Power.(animalID).HbT.(neuralBand).(hemisphere).(behavior).pred_sErr = pred_sErr;
